...
- Stage1 defaults to the 5-preset broad degradation set in `src/datasets/stage1_5preset/`.
...
Stage1 provides two degradation preset sets:

```text
src/datasets/stage1_3preset/  # mild / medium / heavy strength curriculum
src/datasets/stage1_5preset/  # type coverage: clean, standard, blur, noise/JPEG, severe mixed
```

Use the compact 3-preset version:

```bash
STAGE1_PRESET_SET=3 GPU=0 RUN_NAME=focus_sr_main_3preset bash scripts/train_focussr.sh
```

Use the default 5-preset version explicitly:

```bash
STAGE1_PRESET_SET=5 GPU=0 RUN_NAME=focus_sr_main_5preset bash scripts/train_focussr.sh
```

For custom degradation curricula, override both paths and probabilities:

```bash
STAGE1_DEG_FILE_PATHS="path/to/a.yml,path/to/b.yml" \
STAGE1_DEG_PRESET_PROBS="0.6,0.4" \
GPU=0 RUN_NAME=focus_sr_custom_deg bash scripts/train_focussr.sh
```
...
src/datasets/params.yml           # Stage2 Real-ESRGAN degradation preset
src/datasets/stage1_3preset/      # 3-preset Stage1 degradation curriculum
src/datasets/stage1_5preset/      # 5-preset Stage1 broad degradation curriculum
```
