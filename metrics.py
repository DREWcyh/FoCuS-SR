...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to save structured metric results. Defaults to <log>/metrics.json."
    )
    ...
    json_path = args.json or os.path.join(args.log, "metrics.json")
    ...
    for dir_idx, init_dir in enumerate(args.inp_imgs):
        ...
        num_valid_images = sum(1 for item in image_results if not item.get("skipped", False))
        if num_valid_images == 0:
            logger.warning(f"No valid images found for directory: {dir_name}. Skipping averages and FID.")
            results["directories"].append({
                "name": dir_name,
                "sr_dir": init_dir,
                "gt_dir": gt_dir,
                "num_gt_images": len(img_gt_list),
                "num_sr_images": len(img_sr_list),
                "num_valid_images": 0,
        ...
        avg_metrics = {k: round(v / num_valid_images, 4) for k, v in metrics_accum.items()}
    ...
    logger.info(f"Structured metric results saved to: {json_path}")
